...
def country_with_max_renewable(energy: pd.DataFrame) -> tuple:
    idx = energy["% Renewable"].idxmax()
    country = energy.loc[idx, "Country"]
    max_value = energy.loc[idx, "% Renewable"]

    print(f"Country with max % Renewable: {country} ({max_value}%)")
    return country, max_value

res = country_with_max_renewable(energy=energy)
