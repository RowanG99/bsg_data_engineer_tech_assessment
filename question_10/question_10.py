...
for _, row in ScimEn_energy_GDP_merge.iterrows():
    cursor.execute("""
        UPDATE all_countries
        SET continent=?, renewable_bin=?, percent_renewable=?, energy_supply=?, energy_supply_per_capita=?, updated_at=?, deleted_at=NULL
        WHERE country_name=?
    """, (row["Continent"], str(row["RenewableBin"]), row["% Renewable"], row["Energy Supply"], row["Energy Supply per Capita"], datetime.now(), row["Country Name"]))
    
    if cursor.rowcount == 0:
        cursor.execute("""
            INSERT INTO all_countries (country_name, continent, renewable_bin, percent_renewable, energy_supply, energy_supply_per_capita)
            VALUES (?, ?, ?, ?, ?, ?)
        """, (row["Country Name"], row["Continent"], str(row["RenewableBin"]), row["% Renewable"], row["Energy Supply"], row["Energy Supply per Capita"]))

active_countries = tuple(ScimEn_energy_GDP_merge["Country Name"].tolist())
cursor.execute(f"""
    UPDATE all_countries
    SET deleted_at=?
