...
def mean_energy_supply_per_capita(energy: pd.DataFrame) -> float:
    mean_value = energy["Energy Supply per Capita"].mean()
    print(f"Mean Energy Supply per Capita: {mean_value:.2f}")
    return mean_value

res = mean_energy_supply_per_capita(energy=energy)
